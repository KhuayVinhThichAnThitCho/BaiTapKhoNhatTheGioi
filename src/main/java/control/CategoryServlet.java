...
public class CategoryServlet extends HttpServlet{
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
	    ...
	    if (uri.contains("category/add")) {
	        ...
	        req.getRequestDispatcher("/view" + rolePath(user) + "/category-form.jsp").forward(req, resp);
	        ...
	        if (user.getRoleId() == 2) { // manager
	            list = categoryService.findByUserId(user.getUserId());
	        } else {
	            list = categoryService.findAll();
	        }
	        req.setAttribute("categories", list);
	        req.setAttribute("role", rolePath(user).replace("/", ""));
	        req.getRequestDispatcher("/view" + rolePath(user) + "/category-list.jsp").forward(req, resp);
	    }
    }
