...
<body>
    ...
    <form method="post" action="${pageContext.request.contextPath}/${role}/category/${empty category.categoryId ? 'add' : 'edit'}">
        <c:if test="${not empty category.categoryId}">
            <input type="hidden" name="id" value="${category.categoryId}"/>
        </c:if>

        <label for="name">Tên Category:</label>
        <input type="text" name="name" id="name" value="${category.categoryName}" required/>
